...
qv = plt.quiver(slc.lat, slc.level, slc3, -1e1 * slc.values, pivot='mid', scale=2, color='red')
plt.quiverkey(qv, X=.75, Y=-0.1, U=0.5, label='0.5 m s$^{-1}$ 0.05 Pa s$^{-1}$', labelpos='E')
plt.xlabel('Longitude [°]')
plt.ylabel('p [hPa]')
plt.title('%s %s\nShading: $\omega_{QG}\hspace{0.5} [Pa \hspace{0.5} s^{-1}]$; Contours: $\omega_{RA}$; Vectors: in-plane $\\vec{v}_{ag}$ (QG approx.)' % ('117-124°E', '28 Jun–12 Jul'))
plt.yscale('log')
plt.yticks(PTICKS, labels=PTICKS)
plt.gca().invert_yaxis()
