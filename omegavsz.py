...
qgomega = ds.OMEGAQG
omega = ds.OMEGA
slc = qgomega.sel(lat=45, lon=282.5)
...
slc = ds.OMEGAQG.sel(lat=45, lon = slice(230, 300))
...
qv = plt.quiver(slc.lon, slc.level, slc3, -1e1 * slc.values, pivot='mid', scale=1e2, color='red')
plt.quiverkey(qv, X=.75, Y=-0.1, U=5, label='5 m s$^{-1}$ 0.5 Pa s$^{-1}$', labelpos='E')
plt.xlabel('Longitude [°]')
plt.ylabel('p [hPa]')
plt.title('%.1f°N %s\nShading: $\omega_{QG}\hspace{0.5} [Pa \hspace{0.5} s^{-1}]$; Contours: $\omega_{RA}$; Vectors: in-plane $\\vec{v}_{ag}$ (QG approx.)' % (slc.lat.values, slc.time.values))
plt.yscale('log')
plt.yticks(PTICKS, labels=PTICKS)
plt.gca().invert_yaxis()
