#Joshua Pan jp872@cornell.edu 20221024
#Updated to use geostrophic instead of total winds in Q-vec computation. Also corrected a du/dy to dv/dy in Q2 computation
...
def main():
   ...
   HGT = xr.open_dataset('%shgt.%s.nc' % (DATADIR, YEAR)).hgt
   OMEGA = xr.open_dataset('%somega.%s.nc' % (DATADIR, YEAR)).omega
   DS = xr.Dataset(data_vars = {'U': U, 'V': V, 'T': T, 'OMEGA': OMEGA, 'HGT': HGT})
   DS = DS.sel(lat=slice(62.5, 27.4))
   DS = DS.reindex(lat = DS.lat.values[::-1]).fillna(0).sel(time = DS.time.values[14])
   ...
   Q = qvec(DS)
   Qx, Qy = Q[0].sel(level=500), Q[1].sel(level=500)
   forceQ = qgqvec(DS)
   fig, ax = plt.subplots(figsize=(10,7), subplot_kw=dict(projection=ccrs.PlateCarree()))
   ax.set_extent([220, 310, 30, 60])
   ax.coastlines()
   cs1 = ax.contourf(Qx.coords['lon'].values, Qx.coords['lat'].values[1:-1], forceQ[4, ...], cmap = 'BrBG', norm=colors.TwoSlopeNorm(vcenter=0))
   cs = ax.quiver(Qx.coords['lon'].values, Qx.coords['lat'].values, Qx.values, Qy.values, pivot='mid')
   cbar = fig.colorbar(cs1, shrink=0.7, orientation='horizontal')
   plt.title('Q-vectors and Q-vector forcing 500 hPa %s' % str(DS.time.values))
   cbar.set_label('$Pa \hspace{0.5} m^{-2} \hspace{0.5} s^{-1}$')
   plt.savefig('Qvec.png', bbox_inches='tight')
   plt.close()
   ...
   qminusqg = forceQ - qgforcing
   fig, ax = plt.subplots(figsize=(10,7), subplot_kw=dict(projection=ccrs.PlateCarree()))
   ax.set_extent([220, 310, 30, 60])
   ax.coastlines()
   cs = ax.contourf(qminusqg.coords['lon'].values, qminusqg.coords['lat'].values, qminusqg.sel(level=500).values, cmap = 'bwr', norm=colors.TwoSlopeNorm(vcenter=0))
   cbar = fig.colorbar(cs, shrink=0.7, orientation='horizontal')
   cbar.set_label('$Pa \hspace{0.5} m^{-2} \hspace{0.5} s^{-1}$')
   plt.title('Q-vector forcing minus DVA + TA forcing (positive forces ascent) 500 hPa %s' % str(DS.time.values))
   plt.savefig('Qvectrad.png', bbox_inches='tight')
   plt.close()
   ...
   m = forceQ.shape[1] + 1
   n = forceQ.shape[2] - 1
   forcevec = np.reshape(forceQ, (l-1)*(m-1)*(n+1), order='C') #vectorize forcing terms (last axis changing fastest in level,lat,lon)
   #handle the north/south boundary conditions
   for r in range(forcevec.shape[0]):
      i = 1 + r // (n+1) // (m-1)
      j = 1 + r // (n+1) % (m-1)
   ...
   qgQomega = xr.DataArray(omegavec, coords = qgforcing.coords, dims = qgforcing.dims)
   clabelkwargs = {'inline': 1, 'fontsize': 10, 'colors': 'black'}
   contourkwargs = {'colors': 'black', 'transform': ccrs.PlateCarree(), 'levels': np.arange(-5, 5.1, 0.2)}
   fig, ax = plt.subplots(figsize=(10,7), subplot_kw=dict(projection=ccrs.PlateCarree()))
   ax.set_extent([220, 310, 30, 60])
   ax.coastlines()
   cs = ax.contourf(qgQomega.coords['lon'].values, qgQomega.coords['lat'].values, qgQomega.sel(level=500).values, cmap = 'BrBG_r', levels=15, norm=colors.TwoSlopeNorm(vcenter=0))
   cbar = fig.colorbar(cs, shrink=0.7, orientation='horizontal')
   cbar.set_label('$Pa \hspace{0.5} s^{-1}$')
   cs1 = ax.contour(DS.lon.values, DS.lat.values, DS.OMEGA.sel(level=500).values, **contourkwargs)
   ax.clabel(cs1, **clabelkwargs)
   plt.title('Colors: $\omega_{QG}$ (forcing by Q-vectors) 500 hPa %s\nContours: Reanalysis $\omega \hspace{0.5} [Pa \hspace{0.5} s^{-1}]$' % str(DS.time.values))
   plt.savefig('Qvec_omega.png', bbox_inches='tight')
   plt.close()
...
def vg(DS):
   #compute geostrophic wind vector given Dataset containing geopotential height
   pgx, pgy = grad(DS.HGT, DS.dx)
   dakwargs = dict(dims=DS.HGT.dims, coords=DS.HGT.coords)
   return xr.DataArray(-pgy / f0, **dakwargs), xr.DataArray(pgx / f0, **dakwargs)
...
def qvec(DS):
   #compute Q-vectors (not forcing term) given Dataset
   ugeo, vgeo = vg(DS)
   gradu = grad(ugeo, DS.dx)
   gradv = grad(vgeo, DS.dx)
   gradT = grad(DS.T, DS.dx)
   xadv = gradu[0] * gradT[0] + gradv[0] * gradT[1]
   yadv = gradu[1] * gradT[0] + gradv[1] * gradT[1]
   fac = -Rd / DS.sigma / DS.level / 100
   return fac * xadv, fac * yadv

def qgqvec(DS):
   #compute Q-vector forcing term given Dataset
   l = DS.level.shape[0] - 1
   m = DS.lat.shape[0] - 1
   slcpmid, slclatmid = slice(1, l), slice(1, m)
   qx, qy = qvec(DS)
   qxx, _ = grad(qx, DS.dx)
   _, qyy = grad(qy, DS.dx)
   return -2 * (qxx + qyy)[slcpmid, slclatmid, :]
