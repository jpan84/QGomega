#Joshua Pan jp872@cornell.edu 202410 update
...
#Boundary conditions: homogeneous Dirichlet in pressure and lat, periodic in lon
#For advection terms, assume vg = v.
#Update: lots of changes, including modular code, vectorization, and diagnosis of the ageo horizontal winds
...
def main():
   ...
   '''#handle the north/south boundary conditions. Commented out to make homogeneous, which provides similar results without cheating.
...
def lapmat(DS, latm=-1):
   ...
   slclatmid = slice(1,m)
   sz = (l-1)*(m+latm)*(n+1)
   rr = np.repeat(np.arange(sz)[:, None], sz, axis=1)
   cc = rr.T
   ...
   dxvec = np.repeat(dxvec, sz, axis=1)

   L = np.zeros((sz,sz), dtype=np.float64)
   ir, jr, kr = idx1Dto3D(rr, l, m, n)
   ic, jc, kc = idx1Dto3D(cc, l, m, n)

   print(ir, ic)

   nsidx = np.where((ic == ir) & (kc == kr) & (np.abs(jc - jr) == 1))
   L[nsidx] = 1 / dy**2
   ewmask = (ic == ir) & (jc == jr) & (np.abs(kc - kr) == 1)
   L += ewmask * 1 / dxvec**2
   selfx = np.diag(1 / dxvec[:, 0]**2)
   L -= 2 * (selfx + 1 / dy**2)
   periodic = np.where((kr == 0) & (kc == n) | (kr == n) & (kc == 0))
   L[periodic] = 1 / dxvec[periodic]**2
